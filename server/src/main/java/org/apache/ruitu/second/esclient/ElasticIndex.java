...
public class ElasticIndex {
    ...
    public ElasticIndex(@Nonnull List<String> partitionKeysNames, @Nonnull List<String> clusteringColumnsNames) {
        ...
        for (int i = 0; i < stringUrls.length; i++) {
            ...
            hosts.add(new HttpHost("http",split[0], 9200));
        }
    ...
    public void indexData(Map<String, Object> maps, String indexName, String primaryKeyValue) {
        // 索引数据
        try {
            addIndexData(indexName, JSON.toJSONString(maps), primaryKeyValue);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
